...
static void query(void)
{
    ...
    gimp_install_procedure("plug-in-colour-match", "Colour Match",
        "Easily map colour shades keeping highlights and fidelity.",
        "Arel Welgan / Aareus Inc.",
        "Copyright B) 2009 Aareus Inc.", "2009",
        "Colour _Match", "RGB*", GIMP_PLUGIN,
    ...
    gint* nreturn_vals, GimpParam** return_vals)
{
    ...
    *return_vals = values;
    ...
    if (param[0].data.d_int32 == GIMP_RUN_INTERACTIVE) {
        ...
        dialog = gimp_dialog_new("Relative Colour Map", "plug-in-relative-colour-map",
            NULL, 0, gimp_standard_help_func, NULL,
        ...
        gtk_box_pack_start(GTK_BOX(outer), (GtkWidget*)preview, TRUE, TRUE,
            0);
        g_signal_connect(preview, "invalidated", G_CALLBACK(update),
            drawable);
        box = gtk_vbox_new(TRUE, 12);
        gtk_container_add(GTK_CONTAINER(outer), box);

        frame = gtk_frame_new("");
        gtk_label_set_markup((GtkLabel*)
                                 gtk_frame_get_label_widget((GtkFrame*)frame),
            "<b><i>Source Colour</i></b>");
        ...
        g_signal_connect_swapped(source_widget, "color-changed",
        ...
        g_signal_connect_swapped(target_widget, "color-changed",
            G_CALLBACK(gimp_preview_invalidate),
            preview);
...
static void update(GtkWidget* widget, GimpDrawable* drawable)
{
    ...
    for (x = 0; x < width * height; x++, data += in.bpp) {
        ...
        while (hsv.h < 0.0)
            hsv.h += 1.0;
        if (sdiff1 != -1)
            hsv.s = ((sdiff1 > 1) & (hsv.s > config.source.s)) ? (1.0 - (1.0f - hsv.s) * sdiff2) : (hsv.s * sdiff1);
        if (vdiff1 != -1)
...
{
    GimpPixelRgn in, out, *rgn[2] = { &in, &out };
    gpointer pn;
    ...
    gimp_drawable_mask_bounds(drawable->drawable_id, &left, &top, &right,
        &bottom);
    gimp_pixel_rgn_init(&in, drawable, left, top, right - left, bottom - top,
        FALSE, FALSE);
    gimp_pixel_rgn_init(&out, drawable, left, top, right - left, bottom - top,
        TRUE, TRUE);
    ...
    for (pn = gimp_pixel_rgns_register2(2, rgn); pn != NULL; pn = gimp_pixel_rgns_process(pn)) {
        ...
        for (y = 0; y < in.h; y++, datai += r, datao += r) {
            for (x = 0; x < in.w; x++, datai += in.bpp, datao += out.bpp) {
                if (run_code != current_running)
                    return;
    ...
    gimp_drawable_update(drawable->drawable_id, left, top, right - left,
        bottom - top);
